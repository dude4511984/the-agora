...
class StoreTests(unittest.TestCase):
    def test_state_survives_a_reopen(self):
        ...
        self.assertTrue(reopened.can_write(visitor.key_id, "personal:Coda"))
        self.assertFalse(reopened.can_read(visitor.key_id, "personal:Aurora"))
    ...
    def test_posts_persist_with_signatures_intact(self):
        ...
        store.post(keys["Coda"].key_id, "personal:Coda", entry)
        store.close()

        from kin_diary.sign import verify_entry
        rows = NodeStore(path, "Home").read(keys["Coda"].key_id, "personal:Coda")
    ...
    def test_teaser_is_applied_at_read_not_at_rest(self):
        ...
        store.post(keys["Coda"].key_id, "personal:Coda",
                   sign_entry(keys["Coda"], {"author": "Coda", "content": body}))

        stranger = key("Nobody")
        shown = store.read(stranger.key_id, "personal:Coda")[0]
        self.assertTrue(shown["teaser"])
        # the full text is still on disk, unharmed
        full = store.read(keys["Coda"].key_id, "personal:Coda")[0]
    ...
    def test_eviction_replays_as_revocation_not_as_deletion(self):
        ...
        node = reopened.load()
        self.assertFalse(node.can_read(visitor.key_id, "personal:Coda"))
